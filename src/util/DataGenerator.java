...
public class DataGenerator {
    ...
    private static void generateBranches() throws IOException {
        try (FileWriter fw = new FileWriter(DATA_PATH + "branches.csv")) {
            fw.write("branch_id,branch_name,address,phone\n");
            for (int i = 1; i <= 20; i++) {
                fw.write(String.format("B%03d,Branch %d,Address %d,090%07d\n", i, i, i, 1000000 + rand.nextInt(9000000)));
            }
        }
    }

    private static void generateMedicines() throws IOException {
        try (FileWriter fw = new FileWriter(DATA_PATH + "medicines.csv")) {
            fw.write("medicine_id,medicine_name,unit,description,manufacturer\n");
            for (int i = 1; i <= 200; i++) {
                fw.write(String.format("M%04d,Medicine %d,tablet,Desc %d,Manu %d\n", i, i, i, 1 + rand.nextInt(10)));
            }
        }
    }

    private static void generatePharmacists() throws IOException {
        try (FileWriter fw = new FileWriter(DATA_PATH + "pharmacists.csv")) {
            fw.write("pharmacist_id,name,branch_id,username,password_hash,role\n");
            for (int i = 1; i <= 100; i++) {
                fw.write(String.format("P%03d,Pharmacist %d,B%03d,user%d,pass%d,STAFF\n", i, i, 1 + rand.nextInt(20), i, i));
            }
        }
    }

    private static void generateStocks() throws IOException {
        try (FileWriter fw = new FileWriter(DATA_PATH + "stocks.csv")) {
            ...
            // Fill up to 4000
            for (; id <= 4000; id++) {
                fw.write(String.format("S%05d,B%03d,M%04d,%d,1\n", id, 1 + rand.nextInt(20), 1 + rand.nextInt(200), 10 + rand.nextInt(200)));
            }
        }
    }

    private static void generateBatchLots() throws IOException {
        try (FileWriter fw = new FileWriter(DATA_PATH + "batch_lots.csv")) {
            ...
            for (int i = 1; i <= 2000; i++) {
                ...
                fw.write(String.format("BL%05d,M%04d,B%03d,%d,%s,1\n", i, 1 + rand.nextInt(200), 1 + rand.nextInt(20), 10 + rand.nextInt(100), expiry));
            }
        }
    }

    private static void generatePrescriptions() throws IOException {
        try (FileWriter fw = new FileWriter(DATA_PATH + "prescriptions.csv")) {
            ...
            for (int i = 1; i <= 2000; i++) {
                ...
                fw.write(String.format("PR%05d,Patient %d,%s,%s,%s,%s,B%03d,1\n", i, i, dob.format(DATE_FMT), created.format(DATE_FMT), expired.format(DATE_FMT), status, 1 + rand.nextInt(20)));
            }
        }
    }

    private static void generatePrescriptionItems() throws IOException {
        try (FileWriter fw = new FileWriter(DATA_PATH + "prescription_items.csv")) {
            fw.write("prescription_item_id,prescription_id,medicine_id,quantity\n");
            for (int i = 1; i <= 5000; i++) {
                fw.write(String.format("PI%05d,PR%05d,M%04d,%d\n", i, 1 + rand.nextInt(2000), 1 + rand.nextInt(200), 1 + rand.nextInt(5)));
            }
        }
    }

    private static void generateDispenseRecords() throws IOException {
        try (FileWriter fw = new FileWriter(DATA_PATH + "dispense_records.csv")) {
            ...
            for (int i = 1; i <= 1500; i++) {
                ...
                fw.write(String.format("DR%05d,PR%05d,P%03d,%s,B%03d\n", i, 1 + rand.nextInt(2000), 1 + rand.nextInt(100), dispensed.format(DATE_FMT), 1 + rand.nextInt(20)));
            }
        }
    }
}
